...
function draw() {
  ...
  fill(color(220,220,220,0.5));
  generateLayer (3, 5, 'y', 0, 400);


  fill('pink');
  fill(color(230,230,230,50.50));
  generateLayer (2, 5, 'x', 0, 600);


  //fill('green');
  fill(color(240,240,240,0.5));
...
function generateLayer (layerID, nbBars, direction, size, amplitude) {
  ...
  position = layers[layerID].position[0];

  // line(mousePos-(position/2),0,(mousePos/2)+(position/2),height);
  // line(mousePos+(position/2),0,(mousePos/2)+(position/2),height);


  line(position+(mousePos/2),0,(mousePos/2)+(position/2),height);
  line(position-(mousePos/2),0,(mousePos/2)+(position/2),height);



  //line(layers[layerID].position[1],0,mousePos,height);
  ...
  if (nbBars > 2) {
    ...
    for (i = 1; i < nbBars; i++) {
      ...
      //line(mousePos+(position/1.5),0,mousePos+position,height);
      ...
      // position = layers[layerID].position[i+1];
      // line(mousePos+(position/1.5),0,mousePos+position,height);
